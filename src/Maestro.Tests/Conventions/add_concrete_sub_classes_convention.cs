...
namespace Maestro.Tests.Conventions
{
	public class add_concrete_sub_classes_convention
	{
		...
		public void should_register_concrete_sub_classes_of_provided_type()
		{
			...
			var container = new Container(x => x.Scan.Types(types).For.ConcreteSubClassesOf<IBaseType>(y => y.Add()));
		...
		public void should_support_instance_configuration()
		{
			...
			var container1 = new Container(x => x.Scan.Types(types).For.ConcreteSubClassesOf(typeof(IBaseType), y => y.Add().Lifetime.Singleton()));
			var instance1 = container1.GetServices<IBaseType>().Single();
			var instance2 = container1.GetServices<IBaseType>().Single();
			instance1.ShouldBe(instance2);

			var container2 = new Container(x => x.Scan.Types(types).For.ConcreteSubClassesOf<IBaseType>(y => y.Add().Lifetime.Singleton()));
