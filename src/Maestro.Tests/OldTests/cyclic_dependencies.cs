...
namespace Maestro.Tests.OldTests
{
  ...
  Maestro.Tests.resolve_type_with_cyclic_dependencies+Alpha");
		...
		{
			var container = new Container(x => x.Add<Alpha>().Type<Alpha>());
  ...
  Maestro.Tests.resolve_type_with_cyclic_dependencies+Alpha");
		...
		{
			var container = new Container(x => x.Add<object>().Type<object>().Intercept((_, ctx) => ctx.CanGetService<Alpha>()));
		...
		public void Get_should_throw_ActivationException_when_instances_are_resolved_via_lambda_factories()
		{
			var container = new Container(x =>
													{
														x.Add<A>().Factory(ctx => new A { B = ctx.GetService<B>() });
