...
namespace Maestro.Tests.Core.Lifetimes
{
	public class container_scoped_lifetime
	{
		...
		{
			var container = new Container(x => x.For<object>().Use.Type<object>().Lifetime.ContainerScoped());
		...
		{
			var container = new Container(x => x.For(typeof(List<>)).Use.Self().Lifetime.ContainerScoped());
		...
		{
			var rootContainer = new Container(x => x.For<Disposable>().Use.Self().Lifetime.ContainerScoped());
